...
from tf_slim.layers import l2_regularizer
import tf_slim
...
def conv2d(input_, output_dim, kernel=4, stride=2, use_sp=False, padding='SAME', scope="conv2d", use_bias=True):
    with tf.variable_scope(scope):
        w = tf.get_variable('w', [kernel, kernel, input_.get_shape()[-1], output_dim],
                            initializer=tf.variance_scaling_initializer(), regularizer=l2_regularizer(scale=0.0001))
        if use_sp != True:
...
def fully_connect(input_, output_size, scope=None, use_sp=False,
  ...
  with tf.variable_scope(scope or "Linear"):
    matrix = tf.get_variable("Matrix", [shape[1], output_size], tf.float32,
                 initializer=tf.variance_scaling_initializer(), regularizer=l2_regularizer(0.0001))
