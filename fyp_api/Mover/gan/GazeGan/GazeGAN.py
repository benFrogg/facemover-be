...
class Gaze_GAN(object):
    ...
    def test(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            batch_num = 1 / self.opt.batch_size
            for j in range(int(batch_num)):
                ...
                output0 = self.Transpose(np.array([output[0]]))
                output1 = self.Transpose(np.array([output[1]]))
                #print(output0)
                #print(output1)
                print(j)
                #save_images(output_concat, '{}/{:02d}.jpg'.format(self.opt.test_sample_dir, j))
                #save_images(output0, '{}/{:02d}.jpg'.format(self.opt.test_sample_dir, j))
                save_images(output1, '{}/{:04d}.jpg'.format(self.opt.test_sample_dir, j+1))
