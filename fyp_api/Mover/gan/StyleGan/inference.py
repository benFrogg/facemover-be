...
if __name__ == '__main__':
    #load model
    weight = torch.load('logs/cartoon/weight.pth', map_location='cpu')
    model = SimpleGenerator()
    model.load_state_dict(weight)
    model.eval()
    
    #Load input image
    name_list = os.listdir('upload_img/input/A')
    #List the format that is allowed
    name_list = [f for f in name_list if '.jpg' or '.png' in f]
    #Make dict if dict is missing
    if not os.path.exists('list/output'):
        os.mkdir('list/output')
    for name in tqdm(name_list):
        load_path = os.path.join('upload_img/input/A', name)
        save_path = os.path.join('list/output', '0001'+ ".png")
